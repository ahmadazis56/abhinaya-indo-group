...
<main class="flex-1 lg:ml-72 bg-slate-50 min-h-screen">
    <div class="p-6 sm:p-8 max-w-3xl mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Logo</h1>
                <p class="text-slate-500 mt-1 text-sm">Update informasi client atau partner logo.</p>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data" class="divide-y divide-slate-100">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($logo['name']); ?>" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-4 focus:ring-brand-600/15 focus:border-brand-600 transition-all font-medium placeholder:text-slate-400">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="category" name="category" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-slate-900 bg-white focus:outline-none focus:ring-4 focus:ring-brand-600/15 focus:border-brand-600 transition-all font-medium">
                                <option value="client" <?php echo $logo['category'] == 'client' ? 'selected' : ''; ?>>Client (Ditampilkan di Our Clients)</option>
                                <option value="partner" <?php echo $logo['category'] == 'partner' ? 'selected' : ''; ?>>Partner (Ditampilkan di Our Partners)</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" placeholder="Keterangan opsional tentang client/partner ini" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-900 focus:outline-none focus:ring-4 focus:ring-brand-600/15 focus:border-brand-600 transition-all font-medium placeholder:text-slate-400 resize-y"><?php echo htmlspecialchars($logo['description']); ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="current-image mb-8 border border-slate-200 rounded-xl p-4 bg-slate-50">
                        <label class="block text-sm font-semibold text-slate-700 mb-3">Logo Saat Ini:</label>
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            <div class="w-full sm:w-auto h-32 aspect-video bg-white flex items-center justify-center rounded-lg shadow-sm border border-slate-200 p-2 overflow-hidden">
                                <img src="../uploads/logos/<?php echo htmlspecialchars($logo['image']); ?>" alt="Current Logo" class="max-w-full max-h-full object-contain">
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="space-y-6">
                        <div>
                            <label for="image" class="block text-sm font-semibold text-slate-700 mb-2">Ganti File Logo</label>
                            ...
                            <div class="relative group cursor-pointer border-2 border-dashed border-slate-300 rounded-2xl hover:border-brand-500 hover:bg-brand-50 transition-all duration-300 w-full text-center overflow-hidden">
                                <input type="file" id="image" name="image" accept="image/*" onchange="previewLogo(event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                ...
                                <div class="px-6 py-12 flex flex-col items-center justify-center pointer-events-none">
                                    ...
                                    <h4 class="text-slate-800 font-bold mb-1 text-lg">Klik atau seret logo baru ke sini</h4>
                                    <p class="text-slate-500 mb-4 text-sm">Biarkan kosong jika tidak ingin mengubah logo.</p>
                        ...
                        <div id="logoPreviewContainer" class="hidden">
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Preview Logo Baru</label>
                            <div class="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center">
                                <div id="logoPreview" class="w-full max-w-sm h-32 flex items-center justify-center overflow-hidden bg-white border border-slate-200 rounded-lg">
